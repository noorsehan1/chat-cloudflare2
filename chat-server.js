...
const C = {
  ...
  INTERVAL_30_MENIT: 1800000,
  MAX_NUMBER: 6,
  BROADCAST_BATCH_SIZE: 20,
  CLEANUP_INTERVAL: 60000,
  MAX_MESSAGES_PER_MINUTE: 60,
  MAX_BROADCAST_QUEUE: 200,
...
class RateLimiter {
  ...
  checkLimit(username) {
    ...
    if (!record || now > record.resetTime) {
      this.userMessages.set(username, { count: 1, resetTime: now + this.windowMs });
      return true;
    }
  ...
  cleanup() {
    ...
    for (const [username, record] of this.userMessages) {
      if (now > record.resetTime) this.userMessages.delete(username);
    }
  }
}
...
export class ChatServer {
  ...
  _startIntervals() {
    ...
    this._mainInterval = setInterval(() => {
      if (!this.closing && !this.isDestroyed) {
        try { this._doMainTask(); } catch(e) { this._errorCount++; }
      }
    }, C.INTERVAL_30_MENIT);
    ...
    this._cleanupInterval = setInterval(() => {
      if (!this.closing && !this.isDestroyed) {
        try { this._cleanupDeadConnections(); } catch(e) { this._errorCount++; }
      }
    }, C.CLEANUP_INTERVAL);

    setInterval(() => {
      try { this.rateLimiter.cleanup(); } catch(e) {}
    }, 60000);
  }
  ...
  _doMainTask() {
    try {
      ...
      for (const [room, clients] of this.roomClients) {
        if (clients && clients.size > 0) { hasActiveRoom = true; break; }
      }
      ...
      for (const [room, clients] of this.roomClients) {
        if (clients && clients.size > 0) {
          try { this._broadcastToRoom(room, numberMsg).catch(() => {}); } catch(e) { this._errorCount++; }
        }
      }

      for (const [room, roomMan] of this.rooms) {
        if (roomMan) {
          try { roomMan.cleanup(); } catch(e) { this._errorCount++; }
        }
      }
    } catch(e) { this._errorCount++; }
  }
  ...
  _cleanupDeadConnections() {
    try {
      ...
      for (const ws of this.wsSet) {
        try {
          if (!ws || ws.readyState !== 1 || ws._closing) wsToRemove.push(ws);
        } catch(e) { wsToRemove.push(ws); }
      }

      for (const ws of wsToRemove) {
        try { this.cleanup(ws).catch(() => {}); } catch(e) {}
      }
            ...
            try {
              if (ws && ws.readyState === 1 && !ws._closing) {
                ...
                if (username) {
                  ...
                  if (conns && conns.has(ws)) validClients.add(ws);
                }
              }
            } catch(e) {}
          }
  ...
  _queueBroadcast(room, msg) {
    ...
    try {
      const key = `${room}`;
      if (!this._broadcastQueue.has(key)) this._broadcastQueue.set(key, []);
      const queue = this._broadcastQueue.get(key);
      if (queue && queue.length < 50) queue.push(msg);
      if (!this._broadcastTimer) {
        this._broadcastTimer = setTimeout(() => {
          try { this._flushBroadcastQueue(); } catch(e) { this._errorCount++; }
        }, 50);
      }
    } catch(e) { this._errorCount++; }
  }
  ...
  _flushBroadcastQueue() {
    ...
    try {
      ...
      for (const [room, messages] of this._broadcastQueue) {
        ...
        for (const msg of toSend) {
          if (msg) {
            try { this._broadcastToRoom(room, JSON.stringify(msg)).catch(() => {}); } catch(e) { this._errorCount++; }
          }
        }
        if (messages.length === 0) this._broadcastQueue.delete(room);
    ...
    } catch(e) { this._errorCount++; } finally {
      if (this._broadcastQueue.size === 0) this._broadcastQueue.clear();
    }
  }

  async broadcast(room, msg) {
    if (this.closing || this.isDestroyed || !room || !msg) return;
    try { this._queueBroadcast(room, msg); } catch(e) { this._errorCount++; }
  }

  safeSend(ws, msg) {
    if (!ws || ws.readyState !== 1 || ws._closing || this._cleaningUp.has(ws) || this.closing || this.isDestroyed) return false;
    try { ws.send(JSON.stringify(msg)); return true; } catch(e) { this.cleanup(ws).catch(() => {}); return false; }
  }
  ...
  updateRoomCount(room) {
    ...
    try {
      ...
      if (count > 0) this.broadcast(room, ["roomUserCount", room, count]);
      return count;
    } catch(e) { return 0; }
  }

  sendAllStateTo(ws, room, excludeSelf = false) {
    if (!ws || !ws.username || ws.readyState !== 1 || ws._closing || this._cleaningUp.has(ws) || this.closing || this.isDestroyed) return;
    const roomMan = this.rooms.get(room);
    if (!roomMan) return;
    try {
      ...
      if (roomMan.getCount() > 0) this.safeSend(ws, ["roomUserCount", room, roomMan.getCount()]);
      ...
      if (allPoints?.length > 0) {
        ...
        if (excludeSelf && selfSeat) filteredPoints = allPoints.filter(p => p.seat !== selfSeat);
  ...
  async cleanup(ws) {
    if (!ws || ws._cleaning || this._cleaningUp.has(ws) || this._isCleaningUp) return;
    ws._cleaning = true;
  ...
  async handleMessage(ws, raw) {
    if (!ws || ws.readyState !== 1 || ws._closing || this._cleaningUp.has(ws) || this.closing || this.isDestroyed) return;
    if (this._processingMessages.has(ws)) return;
    this._processingMessages.add(ws);

    const timeoutId = setTimeout(() => {
      try { if (ws && ws.readyState === 1) this.safeSend(ws, ["error", "Processing timeout"]); } catch(e) {}
    }, C.CONNECTION_TIMEOUT_MS);

    try {
      let str = typeof raw === 'string' ? raw : new TextDecoder().decode(raw);
      if (str.length > C.MAX_MESSAGE_SIZE) { this.safeSend(ws, ["error", "Message too long"]); return; }

      let data;
      try { data = JSON.parse(str); } catch(e) { this.safeSend(ws, ["error", "Invalid JSON"]); return; }
      ...
      if (evt === "chat" || evt === "updatePoint" || evt === "gift" || evt === "rollangak") {
        ...
        const room = args[0];
        if (room && !ROOMS_SET.has(room)) { this.safeSend(ws, ["error", "Invalid room"]); return; }
      }

      try {
        switch (evt) {
          case "setIdTarget2": await this.handleSetId(ws, args[0], args[1]); break;
          case "joinRoom": await this.handleJoin(ws, args[0]); break;
          case "multiJoin": {
            ...
            for (const [roomName, roomMan] of this.rooms) {
              ...
              for (const [seat, seatData] of roomMan.seats) {
                if (seatData?.namauser === multiUsername) { existingSeat = seat; existingRoom = roomName; break; }
              }
            ...
            if (!this.userCountry.has(multiUsername)) this.userCountry.set(multiUsername, ws.clientCountry || "Unknown");
            ...
            try { await this.broadcast(multiRoomname, ["roomUserCount", multiRoomname, roomMan.getCount()]); } catch(e) {}
            break;
          }
          case "exitMulti": {
            ...
            this.wsToUsername.delete(ws);
            this.wsToRoom.delete(ws);
            if (ws.username === targetUsername) { ws.username = null; ws.idtarget = null; }
            break;
          }
          case "setActiveMulti": {
            const targetUsername = args[0];
            ...
            try { if (roomName) await this.broadcast(roomName, ["userActiveChanged", targetUsername, seatNumber]); } catch(e) {}
            break;
          }
          case "updateKursi": {
            ...
            if (updated) {
              ...
              try { await this.broadcast(kursiRoom, ["kursiBatchUpdate", kursiRoom, [[kursiSeat, updatedSeat]]]); } catch(e) {}
            }
            break;
          }
          case "chat": {
            ...
            if (chatMsg && ROOMS_SET.has(chatRoom)) {
              ...
              if (finalMsg.length > 500) finalMsg = finalMsg.substring(0, 500) + "...";
          ...
          case "setMuteType": {
            ...
            if (!rm) break;
            rm.setMuted(muteVal);
            try { await this.broadcast(muteRoom, ["muteStatusChanged", !!muteVal, muteRoom]); } catch(e) {}
          ...
          case "modwarning": {
            ...
            if (modRoom && ROOMS_SET.has(modRoom)) {
              try { await this.broadcast(modRoom, ["modwarning", modRoom]); } catch(e) {}
            }
            break;
          }
          case "getMuteType": {
          ...
          case "onDestroy": await this.cleanup(ws); break;
      ...
      this._errorCount++;
      try { this.safeSend(ws, ["error", "Error: " + (e.message || "Unknown")]); } catch(err) {}
    } finally {
  ...
  async handleSetId(ws, username, isNewUser) {
    ...
    if (isMultiUser && isNewUser === false) {
      ...
      if (existingConns) {
        for (const oldWs of Array.from(existingConns)) {
          if (oldWs && oldWs !== ws && oldWs.readyState === 1) {
            try { await this.cleanup(oldWs); } catch(e) {}
          }
        }
      }
      ...
      if (!this.userCountry.has(username)) this.userCountry.set(username, userCountry);
  ...
  async handleJoin(ws, roomName) {
    ...
    if (seatData) this.safeSend(ws, ["kursiData", roomName, seat, seatData]);
    if (pointData) this.safeSend(ws, ["pointData", roomName, seat, pointData.x, pointData.y, pointData.fast ? 1 : 0]);
  ...
  async fetch(req) {
    ...
    try {
      ...
      if (this.wsSet.size >= C.MAX_GLOBAL_CONNECTIONS) return new Response("Server full", { status: 503 });
  ...
  async webSocketMessage(ws, msg) {
    ...
    try { await this.handleMessage(ws, msg); } catch(e) { this._errorCount++; }
  }

  async webSocketClose(ws) {
    if (!ws) return;
    try { await this.cleanup(ws); } catch(e) { this._errorCount++; }
  }
  ...
  async destroy() {
    ...
    for (const timeout of this._pendingTimeouts) { clearTimeout(timeout); }
